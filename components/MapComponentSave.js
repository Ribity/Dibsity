...
import { setPannedMap, setPannedCoords} from '../actions/TasksActions';
...
class MapComponent extends React.Component {
    ...
    componentDidMount() {
        try {
            ...
            // this.addGeoFirestoreListeners();  This is called at end of the getLocationsAsync func
            // this._watchLocationAsync(); This is called at end of getLocationAsync func
    ...
    componentWillUnmount() {
        try {
            ...
            if (this.geoSubscription) {
                // console.log("remove geoFirestore Listeners");
    ...
    static getDerivedStateFromProps(nextProps, prevState){
        if (prevState.myPannedSpaces !== nextProps.myPannedSpaces) {
            return{myPannedSpaces: nextProps.myPannedSpaces};
        } else return null;
    };
    componentDidUpdate(prevProps, prevState) {
        if (this.props.myPannedSpaces !== prevProps.myPannedSpaces) {
            this.setUserPanned(false);  // False says to NOT re-display the "Scan For more Ribbons" icon
        }
    }
    handleAnimation = () => {
        try {
    ...
    setUserPanned = (bSetPanned) => {
        ...
        this.timeoutId_pan = setTimeout(this.giveControlBackToMap,
            MyDefines.default_user.profile.refocus_seconds * 1000);
    };
    ...
    check_firestore_listener_distance = () => {
        ...
        // console.log("Check firestore listener meters: ", reset_distance_meters);
        ...
        if (myfuncs.isLocationValid(location) && myfuncs.isLocationValid(firestoreListenerLocation)) {
            ...
            // console.log("Meters since last firestore listener: ", distance)
    ...
    refresh_map_and_firestore_listener = () => {
        // console.log("Reset map and firestore listener");
    ...
    addGeoFirestoreListeners() {
        try {
            ...
            const kilometers = MyDefines.default_client_parms.firestore_map_radius_miles * 1.6;
            ...
            // console.log("Adding firestore query for ", MyDefines.default_client_parms.firestore_map_radius_miles, "miles");
            // console.log(MyDefines.default_client_parms);
            ...
            // firestoreListenerLocation = Object.assign({},location);
    ...
    onPressPannedSpace = (space) => {
        try {
            myfuncs.myBreadCrumbs('onPressPannedSpace', this.props.navigation.state.routeName);

            Alert.alert(space.name, 'id:' + space.id,
                [
                    {
                        text: 'Ribbon Details', onPress: () => {
                            this.goToRibbonDetails(space)
    ...
    render() {
        try {
            ...
            // console.log(this.state.myPannedSpacess);
                ...
                this.state.locationResult === null ?
                    ...
                    (this.state.initRegion === null) ?
                        ...
                        >
                            ...
                            {this.userHasControl &&
                                this.state.myPannedSpaces.map((space, index) => (
                                    <MapView.Marker
                                        key={index}
                                        coordinate={{longitude: space.curr_lon, latitude: space.curr_lat}}
                                        onPress={() => this.onPressPannedSpace(space)}
                                    >
                                        <Image source={space.image_url}
                                               style={{
                                                   width: 8 * MyDefines.default_user.profile.map_ribbon_size,
                                                   height: 11 * MyDefines.default_user.profile.map_ribbon_size,
                                               }}
